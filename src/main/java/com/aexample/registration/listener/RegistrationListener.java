package com.aexample.registration.listener;
...
public class RegistrationListener implements ApplicationListener<OnRegistrationCompleteEvent> {
    ...
    private void confirmRegistration(final OnRegistrationCompleteEvent event) {
        final Accounts account = event.getUser();
        final String token = UUID.randomUUID().toString();
        regService.createVerificationTokenForAccount(account, token);
    ...
    private final SimpleMailMessage constructEmailMessage(final OnRegistrationCompleteEvent event, final Accounts account, final String token) {
